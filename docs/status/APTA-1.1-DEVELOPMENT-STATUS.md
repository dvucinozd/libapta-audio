...
| 7. Native meter/downbeat | Complete implementation / no promoted lattice candidate | Bounded 3/4 vs 4/4 plumbing is complete. A conservative opt-in 3-band phase experiment adds four correct downbeats with zero breaks across 140 already-open development tracks, but absolute accuracy remains far below the release gate. Three temporal-lattice selectors are rejected; the latest beat-path oracle improved Ballroom but failed ASAP transfer. Both formal holdouts remain closed |
| 8. Native musical key | New front-end candidate pre-registered | Bounded global major/minor analysis is complete. Harmonic projection, centered correlation, temporal chord-state voting and soft equal-window profile aggregation were rejected and their splits are spent. A fifth candidate changes the previously fixed tonal front end by integrating three equal probes per semitone on a new disjoint, class-balanced 72-track FMAK split; selected audio remains unopened and the 48-track formal holdout remains unopened; see [`APTA-1.1-WP4-FMAK-SEMITONE-BAND-KEY-PROTOCOL.md`](APTA-1.1-WP4-FMAK-SEMITONE-BAND-KEY-PROTOCOL.md) |
