...
| 8. Native musical key | Complete implementation / WP4 independent transfer reopened | Bounded global major/minor analysis is complete. KP profiles plus log compression remain the production path at 20/60 on the spent human corpus. The opt-in harmonic projection reaches 22/60 safely and remains disabled. After both ASAP label derivations proved nonviable, a new balanced GiantSteps-MTG development/one-shot-holdout protocol was pre-registered before analyzer execution |
