...
| 8. Native musical key | Complete implementation / replacement candidate still open | Bounded global major/minor analysis is complete. Harmonic projection was rejected at 22/96. Resource-neutral centered correlation improved a separate balanced development split from 22/96 to 28/96 and reduced mode bias, but failed all absolute accuracy gates and added 52 new high-confidence errors. The 48-track formal holdout remains unopened; another candidate requires a new pre-registration and genuinely new development evidence |
