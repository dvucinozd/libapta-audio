...
| 8. Native musical key | Complete implementation / temporal candidate pre-registered | Bounded global major/minor analysis is complete. Harmonic projection was rejected at 22/96, and centered correlation was rejected at 28/96 with 52 new high-confidence errors. A third, opt-in temporal chord-state candidate is now frozen before first use on a new 96-track, exactly class-balanced FMAK development split. The 48-track formal holdout remains unopened; see [`APTA-1.1-WP4-FMAK-TEMPORAL-CHORD-KEY-PROTOCOL.md`](APTA-1.1-WP4-FMAK-TEMPORAL-CHORD-KEY-PROTOCOL.md) |
