...
apta_status_t apta_internal_result_pool_create_session_result_meter_base(
    ...
    apta_result_t **result_out)
{
    ...
    status = apta_internal_result_pool_create_session_result_meter_base(
        pool,
        session,
        generation,
        changed_features,
    ...
    status = apta_internal_meter_pool_build(pool, session, *result_out);
    if (status < 0) {
