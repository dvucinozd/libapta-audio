...
apta_status_t apta_internal_waveform_process_meter_base(
...
int apta_internal_analysis_pending_meter_base(const apta_session_t *session);
    ...
    uint32_t *published_output_out)
{
    ...
    apta_status_t meter_status;
    apta_feature_mask_t pending;
    uint32_t remaining_steps;
    uint32_t meter_steps = 0u;
    ...
    status = apta_internal_waveform_process_meter_base(
        session,
        budget,
        work_progress,
        did_work_out,
    ...
    meter_status = apta_internal_meter_refresh(
        session,
        remaining_steps,
        &meter_steps);
    if (meter_status < 0) {
    ...
    work_progress->completed_steps += meter_steps;
    if (meter_steps != 0u) {
        *did_work_out = 1u;
    ...
    if (meter_status == APTA_STATUS_MORE_WORK) {
        status = APTA_STATUS_MORE_WORK;
    }

    pending = apta_internal_meter_pending_features(session);
    if (pending != 0u) {
        ...
        apta_internal_meter_mark_published(session);
...
{
    return apta_internal_analysis_pending_meter_base(session) ||
           apta_internal_meter_refresh_pending(session);
}
