...
#if defined(_MSC_VER)
#define APTA_S4_TRANSIENT_I3_NOINLINE __declspec(noinline)
#elif defined(__GNUC__) || defined(__clang__)
#define APTA_S4_TRANSIENT_I3_NOINLINE __attribute__((noinline))
#else
#define APTA_S4_TRANSIENT_I3_NOINLINE
#endif

static float apta_s4_transient_i3_flux(
    const apta_session_t *session,
    uint64_t bin_index,
    apta_internal_transient_i3_state_t *state)
{
    ...
    frame.broadband =
        ...
        ((float)bin->sample_count *
         (float)APTA_INTERNAL_S4_BAND_MAGNITUDE_SCALE);
    return apta_internal_transient_i3_process(state, &frame);
}

static APTA_S4_TRANSIENT_I3_NOINLINE void apta_s4_fill_transient_i3_flux(
    apta_session_t *session,
    uint64_t evidence_first,
    uint64_t evidence_end)
{
    apta_internal_transient_i3_state_t state = {0};
    uint64_t index;
