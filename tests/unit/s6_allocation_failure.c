...
static int build_valid_file(uint8_t output[FILE_CAPACITY], size_t *size_out)
{
...
static int parse_case(
    const uint8_t *file,
    size_t file_size,
    uint32_t fail_at_call,
    uint32_t *allocation_calls_out)
{
...
cleanup:
    if (allocation_calls_out != NULL) {
        *allocation_calls_out = state.allocation_call;
    }
    if (result != NULL) {
        apta_result_release(result);
    }
...
int main(void)
{
    uint8_t file[FILE_CAPACITY];
    size_t file_size = 0u;
    uint32_t allocation_calls = 0u;
    uint32_t fail_at;

    CHECK(build_valid_file(file, &file_size));
    CHECK(parse_case(file, file_size, 0u, &allocation_calls));
    CHECK(allocation_calls >= 2u);
    for (fail_at = 2u; fail_at <= allocation_calls; ++fail_at) {
        CHECK(parse_case(file, file_size, fail_at, NULL));
    }
    return 0;
}
